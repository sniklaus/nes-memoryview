...
extern "C" void EMSCRIPTEN_KEEPALIVE ram(unsigned char* charCpu, unsigned char* charPpu) {
    if (objRom == NULL) { 
        return;
    }

    std::memcpy(charCpu, objCpu->ram.ram, 2048);
    std::memcpy(charPpu, objPpu->vram, 2048);
}
